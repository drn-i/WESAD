...
def parse_drop_list(drop_cols, drop_cols_from_file):
    # Start from CLI list (may be None)
    drop_set = set(drop_cols or [])
    # Optionally add from file (comma/newline/whitespace-separated)
    if drop_cols_from_file:
        p = Path(drop_cols_from_file)
        if not p.exists():
            raise FileNotFoundError(f"--drop-cols-from-file not found: {p}")
...
def main():
    ...
    # New: feature dropping controls
    ap.add_argument("--drop-cols", nargs="*", default=None, help="Feature columns to drop (space-separated)")
    ap.add_argument("--drop-cols-from-file", default=None, help="Path to a file listing columns to drop (comma or newline separated)")
    args = ap.parse_args()
    ...
    # Prepare features DF
    X_df = df.drop(columns=["label", "subject"])
    X_df = X_df.select_dtypes(include=[np.number])

    # Determine columns to drop and apply
    drop_list = parse_drop_list(args.drop_cols, args.drop_cols_from_file)
    present_to_drop = [c for c in drop_list if c in X_df.columns]
    if present_to_drop:
        X_df = X_df.drop(columns=present_to_drop, errors="ignore")
        # Save dropped columns for traceability
        (out / "dropped_features.txt").write_text("\n".join(sorted(present_to_drop)), encoding="utf-8")
    else:
        # Ensure file exists even if none dropped
        (out / "dropped_features.txt").write_text("", encoding="utf-8")

    if X_df.shape[1] == 0:
        raise ValueError("No features remain after dropping columns. Adjust --drop-cols.")

    feature_names = X_df.columns.tolist()
    X = X_df.values

    # Save correlations (numeric only) after dropping
